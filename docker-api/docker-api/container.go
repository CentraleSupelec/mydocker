...
func canCreateStudentVolume(dockerClient createStudentVolumeDockerClient, request *pb.ContainerRequest) (bool, error) {
	...
	case string(NFS):
		return true, nil
	case string(LOCAL):
		return false, errors.New("unable to create student volume for local storage backend")
	case string(RBD):
		ctx := context.TODO()
		...
		for _, service := range services {
			for _, serviceMount := range service.Spec.TaskTemplate.ContainerSpec.Mounts {
				if serviceMount.Source == createStudentVolumeName(request.UserID) {
					return false, fmt.Errorf("unable to create student volume because there is a conflicting service %s", service.Spec.Name)
				}
			}
		}
		return true, nil
